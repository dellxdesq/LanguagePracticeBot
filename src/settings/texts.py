...
hello_text_1 = (
    ...
    "Например, ты можешь сказать: 'J'aimerais vous parler de littérature en français' или 'Ich möchte auf Deutsch über Musik sprechen.' "
    "Я постараюсь подстроиться под любой язык, который ты выберешь.\n\n"
    "Если я начну говорить несвязно или смешиваю языки, ты можешь очистить историю нашего общения, чтобы я мог говорить более корректно. "
    "Для этого напиши: /clear.\n\n"
)
...
hello_text_2 = (
    ...
    "Без уточнений я тоже постараюсь понять, на каком языке ты хочешь общаться, но лучше сразу указать язык, чтобы разговор был еще более комфортным.\n\n"
    "Не стесняйся уточнять язык и тему, и я постараюсь сделать разговор интересным!\n\n"
    "И помни, если вдруг возникнут проблемы с языками, ты всегда можешь очистить историю, написав /clear."
)
